"""
Hidden Markov Model + Monte Carlo simulation for post count prediction.

States represent Trump's posting "regimes":
  - State 0: Low activity (quiet days, travel, etc.)
  - State 1: Medium activity (normal baseline)
  - State 2: High activity (major news, rallies, controversies)

The HMM learns transition probabilities between these states and emission
distributions (how many posts per day in each state). Monte Carlo then
...
def train_hmm(daily_counts: list[int]) -> hmm.GaussianHMM | None:
    """Train a Gaussian HMM on daily post counts."""
...
def monte_carlo_predict(
    ...
    """
    Run Monte Carlo simulation to predict total posts over a window.

    For each simulation:
    1. Start from current_state
    2. For each day in window, sample next state from transition matrix
    3. Sample daily post count from that state's emission distribution
    4. Sum up total posts for the window
...
def run_predictions() -> list[dict] | None:
    ...
    for window in DEFAULT_PREDICTION_WINDOWS:
        ...
        print(f"\n{window}-day forecast:")
        print(f"  Mean:   {pred['mean']:.1f} posts")
        print(f"  Median: {pred['median']:.1f} posts")
...
def predict_polymarket_threshold(
    ...
    """
    Probability that Trump posts >= threshold times in window_days.
...
def predict_date_range(start_date: str, end_date: str, threshold: int | None = None) -> dict | None:
    """
    Predict post count for a specific date range (e.g., '2026-03-09' to '2026-03-12').
    """
    ...
    print(f"\nSummary Stats:")
    print(f"  Mean:   {result['mean']:.1f} posts")
    print(f"  Median: {result['median']:.1f} posts")
...
def generate_probability_buckets(distribution: np.ndarray, n_buckets: int = 10) -> list[dict]:
    ...
    buckets = []
    lower = int(min_val - (min_val % bucket_size))
